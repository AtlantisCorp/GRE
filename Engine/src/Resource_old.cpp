...
void Resource::addHolder() const
{
    _mCounter++;
}

void Resource::subHolder() const
{
    if(_mCounter > 0)
        _mCounter--;
...
: _resource(), _RefResource(), _isPersistent(persistent)
{
    
}

ResourceUser::ResourceUser (ResourceUser&& movref)
: _resource(std::move(movref._resource)), _RefResource(std::move(movref._RefResource)), _isPersistent(movref._isPersistent)
{
    if(isPersistent())
    {
...
: _resource(rhs._resource), _RefResource(rhs._RefResource), _isPersistent(rhs._isPersistent)
{
    if(isPersistent())
    {
...
_resource ( r ), _isPersistent(persistent)
{
    ...
    if(isReferenced())
    {
        _RefResource = std::dynamic_pointer_cast<ReferencedResource>(_resource.lock());
    }
}
...
bool ResourceUser::isReferenced() const
{
    auto ptr = _resource.lock();
    if(ptr)
        return ptr->isReferenced();
    return false;
}
